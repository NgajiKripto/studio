...
export function ProfileBanner() {
  ...
  return (
    <div className={cn(
      "relative overflow-hidden p-6 rounded-lg border transition-all animate-in slide-in-from-top-4 duration-500",
      isProfileApplied() 
      ...
      <div className="flex flex-col md:flex-row items-center gap-6 justify-between">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          {!isProfileApplied() ? (
              <Button onClick={applyProfileFilters} className="shadow-lg gap-2 w-full md:w-auto">
              <Sparkles className="h-4 w-4" /> Gunakan Filter Profil
            </Button>
          ) : (
            <span className="text-xs font-bold text-accent uppercase tracking-widest px-4 py-2 bg-accent/10 rounded-lg">
              Filter Profil Aktif
            </span>
          )}
          <Button variant="ghost" size="icon" onClick={() => setIsVisible(false)} className="rounded-lg">
            <X className="h-4 w-4" />
          </Button>
        </div>
      </div>
